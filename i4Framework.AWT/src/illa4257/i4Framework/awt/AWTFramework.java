...
public class AWTFramework extends DesktopFramework {
    static Font font;
    public static final Map<Key, Object> BEST;
    static Map<Key, Object> current;
    ...
    static {
        final HashMap<Key, Object> m = new HashMap<>();
    ...
    private final ConcurrentLinkedQueue<AWTWindow> frames = new ConcurrentLinkedQueue<>();

    public AWTFramework(final String appName) {
        super(appName);
    }
    ...
    public void add(final AWTWindow frame) {
        if (frame == null)
            return;
        synchronized (frames) {
    ...
    public void remove(final AWTWindow frame) {
        if (frame == null)
            return;
        synchronized (frames) {
    ...
    @Override public boolean isUIThread(final Component component) { return EventQueue.isDispatchThread(); }
    @Override public void invokeLater(final Runnable runnable) { EventQueue.invokeLater(runnable); }
    ...
    @Override public FrameworkWindow newWindow(final Window window) { return new AWTWindow(this, window); }
