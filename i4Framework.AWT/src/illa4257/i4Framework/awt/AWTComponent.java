...
public class AWTComponent extends java.awt.Container implements IAWTComponent {
    ...
    public AWTComponent(final Component component) {
        ...
        if (component instanceof Container) {
           listeners2 = new EventListener[] {
                   component.addEventListener(AddComponentEvent.class, e -> {
                       if (e.container != component || IAWTComponent.getComponent(this, e.child) != null)
                           return;
                       final AWTComponent co = new AWTComponent(e.child);
                       add(co, 0);
                       co.repaint();
                   }),
                   component.addEventListener(RemoveComponentEvent.class, e -> {
                       if (e.container != component)
                           return;
                       final AWTComponent co = IAWTComponent.getComponent(this, e.child);
                       if (co == null)
                           return;
                       remove(co);
                       co.dispose();
           ...
           for (final Component co : (Container) component) {
               final AWTComponent c = new AWTComponent(co);
               add(c, 0);
               c.repaint();
           }
        }
        ...
        setDropTarget(IAWTComponent.wrapDropTarget(component));
    ...
    private void updateLS(final StyleSetting ignored) {
        final java.awt.Container p = getParent();
        final Insets insets = p instanceof Frame ? p.getInsets() : null;
        final int bw = getBorderWidth(), o = p instanceof AWTComponent ? ((AWTComponent) p).getBorderWidth() : 0;
        setBounds(component.startX.calcInt() - bw + o + (insets != null ? insets.left : 0), component.startY.calcInt() - bw + o + (insets != null ? insets.top : 0), component.width.calcInt() + bw * 2, component.height.calcInt() + bw * 2);
    ...
    public void paint(final Graphics graphics) {
        ...
        g.setRenderingHints(AWTFramework.BEST);
    ...
    @Override public String toString() { return component.toString() + super.toString(); }
}
